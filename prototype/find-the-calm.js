...
function makeRecordedTrack(name, audioFile){
  // Behavior: try to use HTMLAudioElement + MediaElementSource; on error fall back to WebAudio synthetic nodes
  ...
  function createWhiteNoiseBuffer(durationSeconds = 2){
    ...
    const len = Math.max(1, Math.floor(durationSeconds * sr));
  ...
  function ensureNodes(){
    ...
    if (!ctx){
      // not ready; will be created later
      return;
    }
    try {
      // attempt media element source
      ...
      // create synthetic nodes based on track name
      if (name === 'wind'){
        // brown-ish noise with lowpass
        const buf = createWhiteNoiseBuffer(4);
        ...
        // brighter highpassed noise
        const buf = createWhiteNoiseBuffer(2);
        const src = ctx.createBufferSource();
        src.buffer = buf;
        src.loop = true;
        ...
        // piano-ish pad: some detuned oscillators
        const oscs = [];
        ...
        for (let i=0;i<3;i++){
          ...
          o.frequency.value = 220 * (1 + (i-1)*0.01);
          o.start && o.start(0);
          o.connect(g);
          oscs.push(o);
        }
    ...
    // if we reached here and audio element has error state, consider fallback
    if (!usingFallback && audio && audio.error){
      debugLog('audio element error, switching to fallback for', name, audio.error);
      // attempt fallback creation by disconnecting and creating synthetic nodes
      try {
        source.disconnect();
      } catch(e){}
      usingFallback = true;
      // recreate out and synthetic nodes if needed (simple approach: set out already exists)
      // (for brevity we reuse above fallback creation if needed on next ensureNodes call)
    }
  }

  audio.addEventListener('error', () => {
    debugLog('audio error', name, audio.error ? audio.error.code : 'unknown');
    // fallback will be created on ensureNodes
    setTimeout(() => {
      if (ctx) ensureNodes();
    }, 50);
  });
  ...
  function start(){
    ...
    if (!usingFallback){
      audio.play().catch(e => {
        debugLog('play error', name, e && e.message ? e.message : e, '- Check network connection or file availability');
        // create fallback nodes in case of failure
        if (ctx){
          ensureNodes();
        }
      });
    } else {
      // start any BufferSource fallback nodes
      fallbackNodes.forEach(n => { try { if (typeof n.start === 'function') n.start(0); } catch(e){} });
    }
  }
  ...
  function stop(){
    ...
    // stop fallback nodes
    fallbackNodes.forEach(n => { try { if (typeof n.stop === 'function') n.stop(0); } catch(e){} });
  }
