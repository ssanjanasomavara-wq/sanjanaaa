...
export async function initFirebaseWithConfig(firebaseConfig) {
  ...
  // Use string concatenation to prevent webpack from trying to resolve at build time
  const baseUrl = 'https://www.gstatic.com/firebasejs/9.22.1/firebase-';
  const [{ initializeApp }, authMod, dbMod] = await Promise.all([
    import(/* webpackIgnore: true */ baseUrl + 'app.js'),
    import(/* webpackIgnore: true */ baseUrl + 'auth.js'),
    import(/* webpackIgnore: true */ baseUrl + 'database.js')
  ]);
