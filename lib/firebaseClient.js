...
// - Returns the initialized app, auth, db objects plus the SDK modules (authMod/dbMod/firestoreMod/storageMod)
//   so callers can use modular helpers (createUserWithEmailAndPassword, ref, set, getFirestore, doc, etc.).
//
// Usage:
//   const { app, auth, db, authMod, dbMod, firestore, firestoreMod, storage, storageMod } = await initFirebaseWithConfig(firebaseConfig);
...
const FIREBASE_STORAGE_URL = 'https://www.gstatic.com/firebasejs/9.22.1/firebase-storage.js';
 ...
 * @returns {Promise<{app: any, auth: any, db: any, authMod: any, dbMod: any, firestore: any, firestoreMod: any, storage: any, storageMod: any}>}
 */
export async function initFirebaseWithConfig(firebaseConfig) {
  ...
  if (_app && _auth && _db && _authMod && _dbMod && _firestore && _firestoreMod && _storage && _storageMod) {
    return { app: _app, auth: _auth, db: _db, authMod: _authMod, dbMod: _dbMod, firestore: _firestore, firestoreMod: _firestoreMod, storage: _storage, storageMod: _storageMod };
  }
  ...
  const [{ initializeApp }, authMod, dbMod, firestoreMod, storageMod] = await Promise.all([
  ...
  try {
    _storage = _storageMod.getStorage ? _storageMod.getStorage(_app) : null;
  } catch (err) {
