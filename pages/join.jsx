 ...
 *    - If Firestore is configured, writes a document to collection "joins"
 *      with { code, name, email, uid?, createdAt }
 ...
 *   styled to match existing pages (login/home look + owner-provided theme).
 *
 * Notes:
 * - This is intentionally simple. You can later add server-side validation,
 *   admin-controlled invite codes, or email confirmations.
 */
...
export default function Join() {
  ...
  async function submitJoin(e) {
    ...
    // If firestore available, write join document
    ...
    if (fm && f) {
      try {
        ...
        const colRef = collection(f, 'joins');
        await addDoc(colRef, {
          code: payload.code,
        ...
        console.error('Failed to record join to Firestore', err);
        // fallback to local
        saveLocalJoin({ ...payload });
        setJoined(true);
        setStatusMsg('Join recorded locally (server save failed).');
      }
    } else {
      // local fallback
      saveLocalJoin({ ...payload });
      setJoined(true);
      setStatusMsg('Join recorded (local-only mode).');
    }
  }
