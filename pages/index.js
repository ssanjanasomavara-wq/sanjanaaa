...
export default function IndexPage() {
  ...
  // Friendly error mapping (same as before)
      ...
      try {
        ...
        onAuthStateChanged(auth, async (user) => {
          ...
          if (user) {
            ...
            // Show home briefly (keeps UI consistent) then navigate to dashboard.
            // Use replace to avoid creating a "back" entry to the auth page.
            showHome();
            try {
        ...
        if (mode === 'resetPassword' && oobCode) {
          ...
          try {
            ...
            resetDescText && setResetDescText && setResetDescText(email ? `Resetting password for ${email}. Choose a new password.` : 'Choose a new password for your account.');
            setResetMessage('');
            showReset();
          } catch (err) {
  ...
  return (
    <div className="page-root" role="main">
      ...
      {/* HOME (profile-like mobile card with cover and avatar) */}
      {view === 'home' && (
      ...
      {/* SIGNUP / FORGOT / RESET views preserved below */}
      {view === 'signup' && (
      ...
      <style jsx>{`
        /* (CSS omitted here for brevity in this response; keep the same CSS you had previously) */
      `}</style>
    </div>
  );
}
